...
for (let i = 0; i < vacation.length; i++){
    let isExist = false;
    let currentWeekend = new Object();
    for (let j = 0; j<cuteVacation.length; j++){
        if (cuteVacation[j].userId === vacation[i].user._id) {
            isExist = true;
            currentWeekend.startDate = vacation[i].startDate;
            currentWeekend.endDate = vacation[i].endDate;
            console.log();
            cuteVacation[j].weekendDates.push(currentWeekend);
            break;
        }
    }
    ...
    let currentVacation = new Object();
    currentVacation.userId = vacation[i].user._id;
    currentVacation.name = vacation[i].user.name;

    currentWeekend.startDate = vacation[i].startDate;
    currentWeekend.endDate = vacation[i].endDate;
    currentVacation.weekendDates = new Array (currentWeekend);

    cuteVacation.push(currentVacation);
}

console.log(cuteVacation);
for (let i = 0; i < cuteVacation.length; i++){
    console.log(JSON.stringify(cuteVacation[i]));
}